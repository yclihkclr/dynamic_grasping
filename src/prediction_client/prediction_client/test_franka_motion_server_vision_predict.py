...
from prediction import motion_pred
...
class PredictionClientAsync(Node):
    ...
    def detect_for_predict(self,duration = 0.9,show_result = False):
        camera_xyz = []
        while len(camera_xyz) <1:
            camera_xyz=self.detect_once(show_result)
        base_xyz = self.hand_eye_transform(camera_xyz[0])
        start_time = time.time()
        current_time = time.time()
        t_from_start=current_time-start_time
        base_xyt_list = []
        base_xyt_list.append([base_xyz[0],base_xyz[1],t_from_start])
        i = 1
        ...
        print('xyt in 0.9s:',base_xyt_list)
        print('numbers of point:',len(base_xyt_list))

        return base_xyt_list 
...
def main(args=None):
    ...
    pre_pick_joint = [0.1219070047704797, -0.4349117891161065, -0.10831800829945948, -2.6618977468463427, -0.12094998324274314, 2.260655142678155, 0.6360120981385311]
    prediction_client.move_to_joints(pre_pick_joint,velscale)

    print("please type anything to continue")
    key = input()
    print("get typed value, start prediction")
    detect_duration = 0.9 #detect 15 poses in delta_t as 0.1s
    base_xyt_list = prediction_client.detect_for_predict(detect_duration)
    xy_list = []
    for base_xyt in base_xyt_list:
        xy_list.append([base_xyt[0],base_xyt[1]])
    pose_xy = np.array(xy_list)
    print("pose_xy is:",pose_xy)

    # pose_xy = np.array([[    0.51416  ,  -0.14563],
    #                     [    0.51633  ,  -0.14563],
    #                     [     0.5088  ,  -0.14563],
    #                     [     0.5086  ,  -0.14563],
    #                     [    0.51619  , -0.14563],
    ...
    # pose_xy = np.array([[    0.50094  ,  -0.12967],
    #                     [    0.55059  ,   -0.1083],
    #                     [    0.54839  , -0.089258],
    #                     [    0.54956  , -0.070293],
    #                     [    0.54795  ,  -0.05227],
    ...
    pose_pred_arr = pose_pred.cpu().numpy()
    target_pose_rpy = [pose_pred_arr[9][0],pose_pred_arr[9][1],0.10-0.115,3.1415926,0,0]
    duration1 = 1.0 - t_last
    ...
    target_pose_rpy[3] = 3.141592653
    target_pose_rpy[4] = 0.0
    target_pose_rpy[5] = 0.0
